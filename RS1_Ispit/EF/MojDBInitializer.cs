...
namespace RS1_Ispit_asp.net_core.EF
{
    public class MojDBInitializer
    {
        public static void Izvrsi(MojContext context)
        {
            ...
            var testSkole = context.Skola.ToList();
            var testPredmeti = context.Predmet.ToList();
            var testOdjeljenjaStavke = context.OdjeljenjeStavka.ToList();
            var takmicenja = new List<Takmicenje>();
            var takmicenjaUcesnici = new List<TakmicenjeUcesnik>();
            for (int i = 0; i < testSkole.Count(); i++)
            {
                foreach (var item in testPredmeti)
                {
                    takmicenja.Add(new Takmicenje
                    {
                        SkolaId = testSkole.MyRandom().Id,
                        PredmetId = item.Id,
                        Datum = DateTime.Now,
                        Zakljucaj = false
                    }); ;
            ...
            context.SaveChanges();
            takmicenja = context.Takmicenje.ToList();
            for (int i = 0; i < takmicenja.Count(); i++)
            {
                for (int j = 0; j < predmeti.Count(); j++)
                {
                    takmicenjaUcesnici.Add(new TakmicenjeUcesnik
                    {
                        TakmicenjeId = takmicenja.MyRandom().Id,
                        OdjeljenjeStavkaId = testOdjeljenjaStavke.MyRandom().Id,
                        Pristupio = true,
            ...
            context.TakmicenjeUcesnik.AddRange(takmicenjaUcesnici);
            context.SaveChanges();
        }
    }
